...
class EloquentProductRepository implements ProductRepository
{
    ...
    public function addStock(Product $p, int $qty, float $price, ?string $desc): StockEntry
    {
        return DB::transaction(function () use ($p, $qty, $price, $desc) {
            $entry = $p->stockEntries()->create(['quantity' => $qty, 'unit_price' => $price, 'description' => $desc,]);
    ...
    public function update(Product $p, array $d): Product
    {
        $p->update($d);
        return $p;
    }
    ...
    public function recordSale(Product $p, int $qty, float $price, ?string $desc): Sale
    {
        ...
        return DB::transaction(function () use ($p, $qty, $price, $desc) {
            $sale = $p->sales()->create(['quantity' => $qty, 'unit_price' => $price, 'description' => $desc,]);
