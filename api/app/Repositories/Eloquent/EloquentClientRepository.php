...
{
    public function paginateWithStats(int $perPage = 40, ?string $month = null): LengthAwarePaginator
    {
        $query = Client::query()->select('id', 'name', 'telephone', 'telegram', 'created_at')
            // total_debt
            ->withSum(['debtRecords as total_debt' => function ($q) use ($month) {
                if ($month) {
                    $q->whereYear('created_at', substr($month, 0, 4))->whereMonth('created_at', substr($month, 5, 2));
                }
            }], 'amount')
            // paid_amount
